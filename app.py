...
with col_button[1]:
    if st.button('🚀 Obtenir mon estimation gratuite !', use_container_width=True):
        if adresse and telephone and email:
            ...
            st.markdown(f'<div class="info-box"><strong>Détails du calcul :</strong> {result["Détails"]}</div>', unsafe_allow_html=True)
            st.balloons()
            st.markdown(f'<div class="success-box"><h3>✅ Estimation reçue !</h3><p><strong>Hakim vous appelle dans les <u>24h</u> pour :</strong></p><ul><li>Une <strong>estimation plus précise</strong> sur site</li><li>Des <strong>conseils personnalisés</strong> pour mettre en avant votre bien</li><li>Une stratégie adaptée pour <strong>{ville} ({code_postal})</strong></li></ul><p><strong>📞 {telephone} | 📧 {email} | 📍 {adresse}</strong></p></div>', unsafe_allow_html=True)
